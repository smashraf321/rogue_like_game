...
//int dant[21][80];
int dat[21][80]; // 2D arrays to store the distances of every node to PC(@) for both tunneling and non tunneling monsters
...
char foggy_terrain[21][80];
bool fog_of_war_status;
//
// class for storing pc and monsters in my waiting list for next turn to move
//
class mon_q_wl
{
  public:
    ...
    int next_turn;
    int status_DOA;
    unsigned char location_i;
    unsigned char location_j;
    int special_info;
...
int valid_user_input_game[24] = {'7','y','8','k','9','u','6','l','3','n','2','j','1','b','4','h','5',' ','Q','<','>','m','f','g'};
int valid_user_input_menu[3] = {27,KEY_UP,KEY_DOWN};
int valid_user_input_teleportation[18] = {'g','r','7','y','8','k','9','u','6','l','3','n','2','j','1','b','4','h'};
//-------------j, i, x, y-------------//
...
void foggy_terrain_updater();

void print_for_teleportation(int star_pos_i, int star_pos_j);
...
int invalid_user_input_teleporatation(int &star_pos_i, int &star_pos_j, int tele_input);
/***************************************************************************************************/
...
int main (int argc, char* argv[])
{
  ...
  path = new char[strlen(getenv("HOME")) + strlen("/.rlg327/dungeon") + 1];
  ...
  waiting_list = new mon_q_wl[(no_of_monsters+1) * sizeof( *waiting_list)];

  monster_generator();

  foggy_terrain_updater();
...
//------------------------------------------------------------------------------------------------------------------------
  ...
  delete[] r;
  delete[] waiting_list;
  delete[] path; // freeing the dynamically allocated variables
...
void gen()
{
  ...
  r = new unsigned char[n*4]; // n*4 as 4 info fields per room
    ...
    else
    {
      if((waiting_list[0].char_PC_M & TELEPATH) == TELEPATH )
      {
        ...
        else
        {
          ...
          {
            // use tdjikstra to go to PC, eat monsters, no need to draw hashes since can only go where there's zero hardness
            tdjikstra_to_PC();
          }
  ...
  for(i=0;i<21;i++)
    {
      for(j=0;j<80;j++)
      {
        //if(j==0)
          ...
          {
            //if(foggy_terrain[i][j] == '@' || foggy_terrain[i][j] == ' ' || foggy_terrain[i][j] == '.' || foggy_terrain[i][j] == '#' || foggy_terrain[i][j] == '<' || foggy_terrain[i][j] == '>' || foggy_terrain[i][j] == '|' || foggy_terrain[i][j] == '-')
                  mvprintw(i+1,j,"%c",foggy_terrain[i][j]);

                  /*
                  if(foggy_terrain[i][j] >= 0 && foggy_terrain[i][j] <= 15)
                    mvprintw(i+1,j,"%x",foggy_terrain[i][j]);
                  else
                    mvprintw(i+1,j,"%c",foggy_terrain[i][j]);*/
          }
      }
    ...
    if(fog_of_war_status == true)
    {
      for(ii = pciy-2; ii <= pciy+2; ii++)
        for(jj = pcjx-2; jj <= pcjx+2; jj++)
        {
          if( ii > 0 && ii < 20 && jj > 0 && jj < 79)
          {
            if(character_map[ii][jj] != 99)
          {
            if(character_map[ii][jj] == '@')
              mvprintw(ii+1,jj,"%c",character_map[ii][jj]);
              //printf("%c",character_map[i][j]);
            else
              mvprintw(ii+1,jj,"%x",character_map[ii][jj]);
          }
          }
...
int invalid_user_input_game(int user_input)
{
  ...
  int star_pos_i,star_pos_j,teleportation_input;
  char m_dir_y[][6] = {"South","Same","North"};
  char m_dir_x[][5] = {"East","Same","West"};
  WINDOW *menu_win;
      ...
      if(!(user_input == '5' || user_input == ' '))
        {
                  if(user_input == '<' && d[pciy][pcjx] == '<')
                    ...
                    else if(user_input == 'f')
                    {
                      menu_stair_active = 1;
                      invalid = 0;
                      fog_of_war_status = !fog_of_war_status;
                      break;
                    }
                    else if(user_input == 'g')
                    {
                      menu_stair_active = 0;
                      mvprintw(0,28,"You're now in teleportation mode");
                      refresh();
                      star_pos_i = pciy;
                      star_pos_j = pcjx;
                      print_for_teleportation(star_pos_i,star_pos_j);
                      do {
                        teleportation_input = getch();
                      } while(invalid_user_input_teleporatation(star_pos_i,star_pos_j,teleportation_input));
...
void print_for_teleportation(int star_pos_i, int star_pos_j)
{
  ...
  for(i=0;i<21;i++)
    ...
    mvprintw(star_pos_i+1,star_pos_j,"*");
    mvprintw(22,0," ");
    refresh();
}

int invalid_user_input_teleporatation(int &star_pos_i, int &star_pos_j, int tel_input)
{
  int i,ii;
  int invalid;
  ...
  for(i = 0; i < 18; i++)
  {
    ...
    else if(tel_input == '7' || tel_input == 'y')
    {
      int ti, tj;
      ti = star_pos_i + di[0];
      tj = star_pos_j + dj[0];
      if( ti > 0 && ti < 20 && tj > 0 && tj < 79)
      {
        star_pos_i += di[0];
        star_pos_j += dj[0];
        print_for_teleportation(star_pos_i,star_pos_j);
    ...
    else if(tel_input == '8' || tel_input == 'k')
    {
      int ti, tj;
      ti = star_pos_i + di[1];
      tj = star_pos_j + dj[1];
    ...
    else if(tel_input == '9' || tel_input == 'u')
    {
      int ti, tj;
      ti = star_pos_i + di[2];
      tj = star_pos_j + dj[2];
    ...
    else if(tel_input == '6' || tel_input == 'l')
    {
      int ti, tj;
      ti = star_pos_i + di[3];
      tj = star_pos_j + dj[3];
    ...
    else if(tel_input == '3' || tel_input == 'n')
    {
      int ti, tj;
      ti = star_pos_i + di[4];
      tj = star_pos_j + dj[4];
    ...
    else if(tel_input == '2' || tel_input == 'j')
    {
      int ti, tj;
      ti = star_pos_i + di[5];
      tj = star_pos_j + dj[5];
    ...
    else if(tel_input == '1' || tel_input == 'b')
    {
      int ti, tj;
      ti = star_pos_i + di[6];
      tj = star_pos_j + dj[6];
    ...
    else if(tel_input == '4' || tel_input == 'h')
    {
      int ti, tj;
      ti = star_pos_i + di[7];
      tj = star_pos_j + dj[7];
...
void foggy_terrain_updater()
{
  // can initialize empty foggy_terrain here too instead of in dbgen
  int i,j;

for(i = pciy-2; i <= pciy+2; i++)
  for(j = pcjx-2; j <= pcjx+2; j++)
  {
    if( i > 0 && i < 20 && j > 0 && j < 79)
    {
      if(character_map[i][j] == 99)
        foggy_terrain[i][j] = d[i][j];
      else
        foggy_terrain[i][j] = character_map[i][j];
    }
