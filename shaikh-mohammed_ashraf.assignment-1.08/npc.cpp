...
void gen_monsters_desc(dungeon *d, int where_from)
{
  uint32_t i;
  npc *m;
  uint32_t room;
  pair_t p;
  uint32_t num_cells;
  int mon_desc_size = d->monster_descriptions.size();
  num_cells = max_monster_cells(d);
  d->num_monsters = d->max_monsters < num_cells ? d->max_monsters : num_cells;


  for (i = 0; i < d->num_monsters; i++) {
    int index = rand()%mon_desc_size;
    uint32_t rrty = (rand()%100);
    bool excd_rrty = rrty > d->monster_descriptions[index].rarity;
    bool uniq = d->monster_descriptions[index].abilities & NPC_UNIQ;
    //replace uniq with ((d->monster_descriptions[index].abilities & NPC_UNIQ) == NPC_UNIQ)
    // in if statements if getting trouble

    /*if(excd_rrty && (!uniq || (uniq && !d->monster_descriptions[index].uniqueness)))
    {
      i--;
      continue;
    }*/
    //rarity n uniqueness check n level check
    if(excd_rrty){
      i--;
      continue;
    }
    if(uniq && d->monster_descriptions[index].uniqueness && where_from == MAINFIRSTTIME){
    //if((d->monster_descriptions[index].abilities & NPC_UNIQ) == NPC_UNIQ && d->monster_descriptions[index].uniqueness){
      i--;
      continue;
    }
    if(uniq && where_from == STAIRSUPNDOWN && d->monster_descriptions[index].uniqueness && !d->monster_descriptions[index].alive){
      i--;
      continue;
    }
    ...
    m->name = d->monster_descriptions[index].name;
    m->desc = d->monster_descriptions[index].description;
    m->color = d->monster_descriptions[index].color[0];
    m->characteristics = d->monster_descriptions[index].abilities;
    //m->symbol = d->monster_descriptions[index].symbol;
    m->symbol = d->monster_descriptions[index].get_symbol();

    m->speed = d->monster_descriptions[index].speed.roll();
    m->hitpoints = d->monster_descriptions[index].hitpoints.roll();
    m->damage = d->monster_descriptions[index].damage;

    m->rarity = d->monster_descriptions[index].rarity;
    d->monster_descriptions[index].uniqueness = true;
