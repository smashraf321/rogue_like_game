...
uint32_t io_teleport_pc(dungeon *d)
{
  ...
  while ((c = getch()) != 'g' && c != '.' && c != 'r') {
    if (charpair(dest)) {
      ...
      actual = d->object_map[dest[dim_y]][dest[dim_x]]->symbol;
    } else {
      switch (mappair(dest)) {
      case ter_wall:
      case ter_wall_immutable:
    ...
    if(actual != '@' && charpair(dest)){
      attron(COLOR_PAIR(d->character_map[dest[dim_y]][dest[dim_x]]->color));
    }
    else{
      if(d->object_map[dest[dim_y]][dest[dim_x]]){
        attron(COLOR_PAIR(d->object_map[dest[dim_y]][dest[dim_x]]->color));
      }
    }
    mvaddch(dest[dim_y] + 1, dest[dim_x], actual);
    if(actual != '@' && charpair(dest)){
      attroff(COLOR_PAIR(d->character_map[dest[dim_y]][dest[dim_x]]->color));
    }
    else{
      if(d->object_map[dest[dim_y]][dest[dim_x]]){
        attroff(COLOR_PAIR(d->object_map[dest[dim_y]][dest[dim_x]]->color));
      }
    }
