...
void delete_dungeon(dungeon *d)
{
  ...
  //memset doesn't actually free the memory
  //loop through full object map to clear.
  for (uint8_t y = 0; y < DUNGEON_Y; y++) {
    for (uint8_t x = 0; x < DUNGEON_X; x++) {
      if(objxy(x, y)){
        delete objxy(x, y);
      }
    }
...
void new_dungeon(dungeon *d)
{
  ...
  //gen_monsters(d);
  for(uint32_t i = 0; i < d->monster_descriptions.size(); i++ ){
    d->monster_descriptions[i].uniqueness = false;
  }
  for(uint32_t i = 0; i < d->object_descriptions.size(); i++ ){
    d->object_descriptions[i].uniqueness = false;
  }
  gen_monsters_desc(d,2);
  obj_factory(d,2);
}
