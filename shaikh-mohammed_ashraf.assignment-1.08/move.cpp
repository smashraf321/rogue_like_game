...
void do_combat(dungeon *d, character *atk, character *def)
{
  ...
  if (def->alive) {
    def->alive = 0;
    // check for speed and symbol and color of def
    //to see which monster in monster_description vector it matches
    // and make bool alive field of monster as false
    int mon_desc_size = d->monster_descriptions.size();
    for(int i = 0; i < mon_desc_size; i++)
    {
      char def_sym = character_get_symbol(def);
      char mon_desc_sym = d->monster_descriptions[i].symbol;
      int32_t def_speed = def->speed;
      int32_t mon_desc_speed = d->monster_descriptions[i].speed.roll();
      uint32_t def_color = def->color;
      uint32_t mon_desc_color = d->monster_descriptions[i].color[0];
      if((def_sym == mon_desc_sym)&&(def_speed == mon_desc_speed)&&(def_color == mon_desc_color)){
        d->monster_descriptions[i].alive = false;
        break;
      }
    }
