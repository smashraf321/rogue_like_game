...
void move_character(dungeon *d, character *c, pair_t next)
{
  ...
  if (charpair(next) &&
    ...
    if(charpair(next) == d->PC || c == d->PC){
      ...
      charpair(next)->hp -= c->damage->roll();

      if(charpair(next)->hp <= 0){
	...
	if(d->character_map[charpair(next)->position[dim_y]+dy[i]][charpair(next)->position[dim_x]+dx[i]] == NULL){
	  ...
	  d->character_map[charpair(next)->position[dim_y]][charpair(next)->position[dim_x]] = charpair(next);
	...
	charpair(next)->position[dim_y] = c->position[dim_y];
	charpair(next)->position[dim_y] = c->position[dim_y];
	...
	d->character_map[c->position[dim_y]][c->position[dim_x]] = d->character_map[charpair(next)->position[dim_y]][charpair(next)->position[dim_x]];
	//c replaced by charpair(next)
	d->character_map[charpair(next)->position[dim_y]][charpair(next)->position[dim_x]] = tmp;
