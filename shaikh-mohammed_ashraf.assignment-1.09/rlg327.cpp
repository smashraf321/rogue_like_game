...
int main(int argc, char *argv[])
{
 ...
 if (argc > 1) {
  ...
  while (pc_is_alive(&d) && !d.game_over && !d.quit) {
    do_moves(&d);
  }
  io_display(&d);
  ...
  // deleting items from inventory and equipped items
  for(int i = 0; i < 12; i++){
    
	if(d.PC->equipped_items[i]){
		delete d.PC->equipped_items[i];
		d.PC->equipped_items[i] = NULL;
    }
    ...
    if(i<10){
	  ...
	  if(d.PC->carry_items[i]){
		...
		delete d.PC->carry_items[i];
		d.PC->carry_items[i] = NULL;
