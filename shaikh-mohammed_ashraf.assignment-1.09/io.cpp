...
static void io_list_equipped_items(dungeon *d)
{
  char ch = 10;
  mvprintw(0, 0, " %-60s ", "");
    mvprintw(3, 9, " %-60s ", "");
    mvprintw(4, 9, " %-60s ", "You're now viewing equipped items!");
    mvprintw(5, 9, " %-60s ", "");

    /* Display it */
    for(int i = 0; i < 12; i++){

        if(d->PC->equipped_items[i]){

	  mvprintw(6 + i, 9, " %c. %-60s ", i+97,d->PC->equipped_items[i]->get_name());

        } else {

            mvprintw(6 + i, 9, " %c. %-60s ", i+97,"");

        }

    }

    mvprintw(18, 9, " %-60s ", "Press Esc to exit!");
...
{
	char ch = 10;
	bool no_slots_empty = true;
	mvprintw(0, 0, " %-60s ", "");
    mvprintw(3, 9, " %-60s ", "");
    mvprintw(4, 9, " %-60s ", "Enter indexing letter to remove equipped item");
    mvprintw(5, 9, " %-60s ", "");
	...
	mvprintw(18, 9, " %-60s ", "If your mind changed, hit Esc to exit");

	while(!((ch > 96 && ch < 109)||(ch == 27))){
	...
	if(ch == 27){
		...
		// proceed to take off equipped item
		io_display(d);

		if(d->PC->equipped_items[ch-97]){

			for(int i = 0; i < 10; i++){

				if(!d->PC->carry_items[i]){

                    d->PC->speed -= d->PC->equipped_items[ch-97]->get_speed();
					d->PC->carry_items[i] = d->PC->equipped_items[ch-97];
					d->PC->equipped_items[ch-97] = NULL;

					mvprintw(3, 9, " %-60s ", "");
					mvprintw(4, 9, " %-60s ", "Item Successfully taken off!");
					mvprintw(5, 9, " %-60s ", "");

					no_slots_empty = false;
					break;
				}
			...
			if(no_slots_empty){
				...
				mvprintw(3, 9, " %-60s ", "");
				mvprintw(4, 9, " %-60s ", "Sorry, no empty carry slots");
				mvprintw(5, 9, " %-60s ", "");
			...
			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Can't unequip what was not equipped!");
			mvprintw(5, 9, " %-60s ", "");

		}

		mvprintw(6, 9, " %-60s ", "Press ESC to exit!");
		while(ch != 27){
...
static void io_list_carry_items(dungeon *d)
{
  char ch = 10;
  mvprintw(0, 0, " %-60s ", "");
    mvprintw(3, 9, " %-60s ", "");
    mvprintw(4, 9, " %-60s ", "You're now viewing inventory!");
    mvprintw(5, 9, " %-60s ", "");
    ...
    for(int i = 0; i < 10; i++){
	  ...
	  mvprintw(6 + i, 9, " %d. %-57s ", i,d->PC->carry_items[i]->get_name());

        } else {

            mvprintw(6 + i, 9, " %d. %-57s ", i,"");

        }

    }

    mvprintw(16, 9, " %-60s ", "Press Esc to exit!");
...
static void io_drop_item(dungeon *d)
{
	...
	mvprintw(0, 0, " %-60s ", "");
	mvprintw(3, 9, " %-60s ", "");
	mvprintw(4, 9, " %-60s ", "Enter item no. from inventory for drop!");
	mvprintw(5, 9, " %-60s ", "");
	...
	mvprintw(16, 9, " %-60s ", "If your mind changed, hit Esc to exit");

	while(!((ch > 47 && ch < 58)||(ch == 27))){
	...
	// proceed with dropping an item
	// can't drop one item on another
		if(d->PC->carry_items[ch-48] && objxy(d->PC->position[dim_x],d->PC->position[dim_y]) == NULL){

			objxy(d->PC->position[dim_x], d->PC->position[dim_y]) = d->PC->carry_items[ch-48];
			d->PC->carry_items[ch-48] = NULL;

			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Item Successfully dropped!");
			mvprintw(5, 9, " %-60s ", "");

		} else {

			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Drop not possible");
			mvprintw(5, 9, " %-60s ", "");
...
static void io_expunge_item(dungeon *d)
{
	...
	mvprintw(0, 0, " %-60s ", "");
	mvprintw(3, 9, " %-60s ", "");
	mvprintw(4, 9, " %-60s ", "Enter item no. from inventory for expunging");
	mvprintw(5, 9, " %-60s ", "");
	...
	// proceed with expunging an item
		io_display(d);

		if(d->PC->carry_items[ch-48]){

			delete d->PC->carry_items[ch-48];
			d->PC->carry_items[ch-48] = NULL;

			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Item Successfully expunged!");
			mvprintw(5, 9, " %-60s ", "");

		} else {

			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Expunge not possible");
			mvprintw(5, 9, " %-60s ", "");
...
static void io_wear_item(dungeon *d)
{
	...
	mvprintw(0, 0, " %-60s ", "");
	mvprintw(3, 9, " %-60s ", "");
	mvprintw(4, 9, " %-60s ", "Enter item no. from inventory for wearing");
	mvprintw(5, 9, " %-60s ", "");
	...
	// proceed with wearing an item
		...
		if(d->PC->carry_items[ch-48]){
			...
			// not looping since only 1 slot for each type
			if(d->PC->carry_items[ch-48]->get_type() != 11){
				// 11 is type number of ring
                if(d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1] != NULL){
                    d->PC->speed -= d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1]->get_speed();
                }
                d->PC->speed += d->PC->carry_items[ch-48]->get_speed();
				tmp = d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1];
				d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1] = d->PC->carry_items[ch-48];
				d->PC->carry_items[ch-48] = tmp;

			} else {
				// iterating through two ring slots
				for(int i = 0; i < 2; i ++){
                    // looking for empty ring slot
					if(d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1+i] == NULL){
                        d->PC->speed += d->PC->carry_items[ch-48]->get_speed();
						d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1+i] = d->PC->carry_items[ch-48];
						d->PC->carry_items[ch-48] = NULL;
						no_empty_slot = false;
				...
				if(no_empty_slot){
					// if no ring slot was empty, swap with first ring
                    d->PC->speed -= d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1]->get_speed();
                    d->PC->speed += d->PC->carry_items[ch-48]->get_speed();
					tmp = d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1];
					d->PC->equipped_items[d->PC->carry_items[ch-48]->get_type()-1] = d->PC->carry_items[ch-48];
			...
			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Item Successfully equipped!");
			mvprintw(5, 9, " %-60s ", "");
			...
			mvprintw(3, 9, " %-60s ", "");
			mvprintw(4, 9, " %-60s ", "Equip not possible");
			mvprintw(5, 9, " %-60s ", "");
...
static void io_list_carry_items_desc(dungeon *d)
{
  char ch = 10;
  ...
  const char *obj_type[] = {
    ...
    mvprintw(0, 0, " %-60s ", "");
  mvprintw(3, 9, " %-60s ", "");
  mvprintw(4, 9, " %-60s ", "Enter item no. from inventory for desc!");
  mvprintw(5, 9, " %-60s ", "");
  ...
  while(!((ch > 47 && ch < 58)||(ch == 27))){
    ...
    if(ch == 27){
      ...
      mvprintw(3, 9, " %-60s ", "");
      mvprintw(4, 9, " %-60s ", "You're now viewing description!");
      mvprintw(5, 9, " %-60s ", "");

      if(d->PC->carry_items[ch-48]){

	mvprintw( 6, 9, " NAME: %-54s ", d->PC->carry_items[ch-48]->get_name());
	mvprintw( 7, 9, " SYMBOL: %-52c ", d->PC->carry_items[ch-48]->get_symbol());
	mvprintw( 8, 9, " TYPE: %-54s ", obj_type[d->PC->carry_items[ch-48]->get_type()]);
	mvprintw( 9, 9, " SPEED: %-53d ", d->PC->carry_items[ch-48]->get_speed());
	mvprintw(10, 9, " DAMAGE: %d + %d d %d %-42s ", d->PC->carry_items[ch-48]->get_damage_base(), d->PC->carry_items[ch-48]->get_damage_number(), d->PC->carry_items[ch-48]->get_damage_sides(),"");

      } else {

	mvprintw(6, 9, " %-60s ","No item found!!");
	mvprintw(7, 9, " %-60s ","");
	mvprintw(8, 9, " %-60s ","");
	mvprintw(9, 9, " %-60s ","");
	mvprintw(10, 9, " %-60s ","");
      ...
      mvprintw(11, 9, " %-60s ", "Press Esc to exit!");
...
static void io_list_monsters_desc(dungeon *d)
{
  pair_t dest;
  int c;
  fd_set readfs;
  ...
  mvprintw(0, 0,
           "Choose a location. 't' to display info about monster ");
  ...
  * not otherwise used.                                                 */
  ...
  if (charpair(dest) && charpair(dest) != d->PC) {
    //display monster description

    //getting string of monster characteristics
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_SMART) == NPC_SMART){
      strcat(abils,"SMART ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_TELEPATH) == NPC_TELEPATH){
      strcat(abils,"TELE ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_TUNNEL) == NPC_TUNNEL){
      strcat(abils,"TUNNEL ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_ERRATIC) == NPC_ERRATIC){
      strcat(abils, "ERRATIC ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_PASS_WALL) == NPC_PASS_WALL){
    strcat(abils,"PASS ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_UNIQ) == NPC_UNIQ){
      strcat(abils, "UNIQ ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_PICKUP_OBJ) == NPC_PICKUP_OBJ){
      strcat(abils,"PICKUP ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_DESTROY_OBJ) == NPC_DESTROY_OBJ){
      strcat(abils,"DESTROY ");
    }
    if((((npc *) d->character_map[dest[dim_y]][dest[dim_x]])->characteristics & NPC_BOSS) == NPC_BOSS){
      strcat(abils,"BOSS ");
    }

    //start displaying desc

    mvprintw( 6, 9, " NAME: %-54s ", charpair(dest)->name );
    mvprintw( 7, 9, " SYMBOL: %-52c ", charpair(dest)->get_symbol());
    mvprintw( 8, 9, " SPEED: %-53d ", charpair(dest)->speed );
    mvprintw( 9, 9, " CHARACTERISTICS: %-42s ", abils);
    mvprintw(10, 9, " HP: %-56d ", charpair(dest)->hp );
    mvprintw(11, 9, " DAMAGE: %d + %d d %d %-42s ", charpair(dest)->damage->get_base(), charpair(dest)->damage->get_number(), charpair(dest)->damage->get_sides(), "" );

  } else {
    //monster not found

    mvprintw(6, 9, " %-60s ", "Monster not found");
    mvprintw(7, 9, " %-60s ", "");
    mvprintw(8, 9, " %-60s ", "");
    mvprintw(9, 9, " %-60s ", "");
    mvprintw(10, 9, " %-60s ", "");
  ...
  mvprintw(12, 9, " %-60s ", "Press ESC to exit");
...
void io_handle_input(dungeon *d)
{
  ...
  do {
    ...
    case 'm':
      ...
      break;
    case 'e':
      io_list_equipped_items(d);
      fail_code = 1;
      break;
    case 'i':
      io_list_carry_items(d);
      fail_code = 1;
      break;
    case 'I':
      io_list_carry_items_desc(d);
      fail_code = 1;
      break;
    case 'L':
      io_list_monsters_desc(d);
      fail_code = 1;
      break;
	case 'd':
		io_drop_item(d);
